...
public:
	...
	unsigned int height;
	TreeNode<K, D>(K key, D* data, TreeNode<K, D> *father) : key(key), data(data), father(father),
	                            left_son(nullptr), right_son(nullptr), balance(0),sub_tree_size(1),  height(0) {}
	~TreeNode() = default;
	// add maybe operators
	...
	void print() {};//todo: remove or implement
...
template <class K, class D>
unsigned int TreeNode<K, D>::getRightSubHeight() const
{
	return this->right_son->getHeight();
}

template <class K, class D>
unsigned int TreeNode<K, D>::getLeftSubHeight() const
{
	return this->left_son->getHeight();
}
